...
const crypto_1 = require("crypto"); // ADDED: for random bytes if needed
...
const makeMessagesSocket = (config) => {
    ...
    // ADDED: Helper function to check if message contains buttons/interactive content
    ...
    // ADDED: Helper function to get biz node for buttons
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k, _l, _m, _o, _p; // MODIFIED: Added more variables
            ...
            // ADDED: Get message content for button detection
            const messageContent = (0, Utils_1.normalizeMessageContent)(message);
            
            // ADDED: Add biz node for buttons/interactive messages
            if (!isNewsletter && (isGroup || (0, WABinary_1.isJidUser)(destinationJid))) {
                const bizNode = getBizNodeForButtons(message);
                if (bizNode) {
                    binaryNodeContent.push(bizNode);
                    logger.debug({ jid }, "adding biz node for buttons/interactive message");
                }
            }
    ...
    // MODIFIED: Enhanced getMessageType to properly detect button/interactive message types
    
const getMessageType = (message) => {
    if (message.buttonsMessage) return "text";
    if (message.buttonsResponseMessage) return "text";
    if (message.listMessage) return "text";
    if (message.listResponseMessage) return "text";
    if (message.interactiveMessage) return "text";
    if (message.interactiveResponseMessage) return "text";
    if (message.nativeFlowMessage) return "text";
...
const getMediaType = (message) => {
    ...
    // if (message.nativeFlowMessage) return "native_flow";
};
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            var _a, _b, _c, _d, _e, _f, _g, _h, _j, _k; // MODIFIED: Added more variables
            const userJid = authState.creds.me.id;
            
            // MODIFIED: Handle group status message first
            ...
            // Handle all other message types (including buttons)
            else {
                // ADDED: Check if content contains buttons or interactive elements
                ...
                // MODIFIED: Generate message with proper upload function
                ...
                // ADDED: Check for button-specific message types
                const isButtonMessage = hasButtons && !isDeleteMsg && !isEditMsg && !isPinMsg && !isPollMessage;
                ...
                // ADDED: For button messages, we might need to set specific attributes
                if (isButtonMessage) {
                    // Buttons typically don't need special edit attributes
                    logger.debug({ jid }, "sending button/interactive message");
                }
