...
const makeMessagesSocket = (config) => {
    ...
    // FIXED: Message type detection with group status support
    const getMessageType = (message) => {
        const normalized = (0, Utils_1.normalizeMessageContent)(message);
        
        // Check for group status messages
        if (normalized?.groupStatusMessage) return "text";
        if (normalized?.groupStatusMessageV2) return "text";
        
        // Check for button-related message types
        if (normalized?.buttonsMessage) return "text";
        if (normalized?.buttonsResponseMessage) return "text";
        if (normalized?.listMessage) return "text";
        if (normalized?.listResponseMessage) return "text";
        if (normalized?.interactiveMessage) return "text";
        if (normalized?.interactiveResponseMessage) return "text";
        if (normalized?.nativeFlowMessage) return "text";
        if (normalized?.templateMessage) return "text";
    ...
    // FIXED: Media type detection with group status support
    const getMediaType = (message) => {
        const normalized = (0, Utils_1.normalizeMessageContent)(message);
        if (!normalized) return;
        
        // Only return actual media types
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            // FIXED: Only add biz node for messages that actually have buttons
            // Skip for group status messages and regular media messages
            if (!isNewsletter && !isGroupStatusMessage(message)) {
                const hasButtons = hasButtonContent(message);
                if (hasButtons) {
                    const bizNode = getBizNodeForButtons(message);
                    if (bizNode) {
                        binaryNodeContent.push(bizNode);
                        logger.debug({ jid }, "adding biz node for buttons");
                        
                        // Ensure message type is set to "text" for buttons
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Handle group status messages FIRST
            ...
            // Handle ALL other messages (including media without buttons and buttons)
