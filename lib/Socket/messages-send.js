...
const makeMessagesSocket = (config) => {
    ...
    const getMessageType = (message) => {
        ...
        // Check for media messages using normalized content
        const mediaType = getMediaTypeFromNormalized(normalized);
        if (mediaType) {
            return "text"; // Media messages use "text" type
        }
    ...
    const getMediaTypeFromNormalized = (normalized) => {
        ...
        // Handle group status messages with media
    ...
    // Keep original getMediaType for backward compatibility but use normalized version internally
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            // CRITICAL FIX: Check if this is a media message
            const normalizedMsg = (0, Utils_1.normalizeMessageContent)(message);
            ...
            // IMPORTANT: Only add biz node for standalone buttons (NO media)
            // Media messages with buttons should NOT get a separate biz node
